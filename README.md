...
3. Login with your credentials. (Default username is `admin`, the password is randomly generated and stored in `secrets.yaml`)
...
3. Connectez-vous avec vos identifiants. (Le nom d'utilisateur par défaut est `admin`, le mot de passe est généré aléatoirement et stocké dans `secrets.yaml`)
