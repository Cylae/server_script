...
*   `core`: Handles low-level system interactions, hardware detection, and secrets generation.
*   `interface`: Provides the CLI and Web Administration Dashboard.
*   `services`: Defines the supported Docker services and their configurations.

## 🔒 Security

Server Manager is designed with security in mind:
*   UFW firewall is automatically configured.
*   Passwords and secrets are generated securely and stored locally.
*   The Web Dashboard uses secure sessions and requires authentication.
*   Docker containers are isolated on separate networks.
