...
3. Login with your credentials. The default `admin` password is randomly generated during installation and printed in the deployment summary (or saved in `/root/credentials.txt`).
...
3. Connectez-vous avec vos identifiants. Le mot de passe `admin` par défaut est généré aléatoirement lors de l'installation et affiché dans le résumé de déploiement (ou sauvegardé dans `/root/credentials.txt`).
