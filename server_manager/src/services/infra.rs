...
"#,
    ...
    fn env_vars(&self, hw: &HardwareInfo, secrets: &Secrets) -> HashMap<String, String> {
        ...
        vars.insert(
            ...
            secrets.mysql_root_password.as_deref().unwrap_or_default().to_string(),
        );
...
impl Service for NginxProxyService {
    ...
    fn volumes(&self, _hw: &HardwareInfo) -> Vec<String> {
        vec![
            "./config/npm:/data".to_string(),
            "./config/npm/letsencrypt:/etc/letsencrypt".to_string(),
        ]
    }
    fn resources(&self, hw: &HardwareInfo) -> Option<ResourceConfig> {
        let memory_limit = match hw.profile {
...
impl Service for WireguardService {
    ...
    fn cap_add(&self) -> Vec<String> {
        vec!["NET_ADMIN".to_string(), "SYS_MODULE".to_string()]
    }
    fn sysctls(&self) -> Vec<String> {
        vec!["net.ipv4.conf.all.src_valid_mark=1".to_string()]
    }
    fn volumes(&self, _hw: &HardwareInfo) -> Vec<String> {
        vec![
            "./config/wireguard:/config".to_string(),
            "/lib/modules:/lib/modules".to_string(),
        ]
    }
    fn resources(&self, hw: &HardwareInfo) -> Option<ResourceConfig> {
        let memory_limit = match hw.profile {
