...
impl Service for VaultwardenService {
    ...
    fn env_vars(&self, _hw: &HardwareInfo, secrets: &Secrets) -> HashMap<String, String> {
        ...
        vars.insert(
            ...
            secrets.vaultwarden_admin_token.as_deref().unwrap_or_default().to_string(),
        );
        vars
    }
...
impl Service for YourlsService {
    ...
    fn env_vars(&self, _hw: &HardwareInfo, secrets: &Secrets) -> HashMap<String, String> {
        ...
        vars.insert(
            ...
            secrets.mysql_user_password.as_deref().unwrap_or_default().to_string(),
        );
        ...
        vars.insert(
            ...
            secrets.yourls_admin_password.as_deref().unwrap_or_default().to_string(),
        );
        vars
    }
...
impl Service for GiteaService {
    ...
    fn env_vars(&self, _hw: &HardwareInfo, secrets: &Secrets) -> HashMap<String, String> {
        ...
        vars.insert(
            "GITEA__database__HOST".to_string(),
            "mariadb:3306".to_string(),
        );
        ...
        vars.insert(
            ...
            secrets.gitea_db_password.as_deref().unwrap_or_default().to_string(),
        );
        vars
    }
    fn volumes(&self, _hw: &HardwareInfo) -> Vec<String> {
...
impl Service for NextcloudService {
    ...
    fn configure(&self, _hw: &HardwareInfo, secrets: &Secrets) -> Result<()> {
        ...
        let db_pass = escape_php(&secrets.nextcloud_db_password.as_deref().unwrap_or_default().to_string());
        let admin_pass = escape_php(&secrets.nextcloud_admin_password.as_deref().unwrap_or_default().to_string());
...
"#,
    ...
    fn env_vars(&self, hw: &HardwareInfo, secrets: &Secrets) -> HashMap<String, String> {
        ...
        vars.insert(
            ...
            secrets.nextcloud_db_password.as_deref().unwrap_or_default().to_string(),
        );
    ...
    fn volumes(&self, _hw: &HardwareInfo) -> Vec<String> {
        vec![
            "./config/nextcloud:/config".to_string(),
            "./media:/data".to_string(),
        ]
    }
    fn depends_on(&self) -> Vec<String> {
        vec!["mariadb".to_string(), "redis".to_string()]
    }
}
