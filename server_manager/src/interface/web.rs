...
async fn add_user_handler(State(_state): State<SharedState>, session: Session, Form(payload): Form<AddUserPayload>) -> impl IntoResponse {
    ...
    if let Err(e) = UserManager::add_user_async(payload.username.clone(), payload.password, role_enum, quota_val).await {
...
async fn delete_user_handler(State(_state): State<SharedState>, session: Session, Path(username): Path<String>) -> impl IntoResponse {
    let session_user: SessionUser = match session.get(SESSION_KEY).await {
        Ok(Some(u)) => u,
        _ => return Redirect::to("/login").into_response(),
    };
    ...
    if let Err(e) = UserManager::delete_user_async(username.clone()).await {
...
async fn check_admin_role(session: Session, name: &str, enable: bool) -> impl IntoResponse {
    ...
    // Update config async
    let res = if enable {
        Config::enable_service_async(name).await
    } else {
        Config::disable_service_async(name).await
    };
    ...
    run_cli_apply(name, enable);
    Redirect::to("/").into_response()
}

fn run_cli_apply(service: &str, enable: bool) {
    let action = if enable { "enabled" } else { "disabled" };
    info!("Web UI triggering apply. Service {} {}", service, action);

    if let Ok(exe) = std::env::current_exe() {
        match Command::new(exe).arg("apply").spawn() {
            Ok(mut child) => {
