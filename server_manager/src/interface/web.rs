...
async fn add_user_handler(State(_state): State<SharedState>, session: Session, Form(payload): Form<AddUserPayload>) -> impl IntoResponse {
    ...
    let res = UserManager::add_user_async(payload.username.clone(), payload.password, role_enum, quota_val).await;
...
async fn delete_user_handler(State(_state): State<SharedState>, session: Session, Path(username): Path<String>) -> impl IntoResponse {
    ...
    let res = UserManager::delete_user_async(username.clone()).await;
