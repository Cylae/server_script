...
async fn test_mock_system_ops() {
    ...
    assert!(ops.vacuum_journal("100M").is_ok());
    ...
    assert_eq!(calls[4], "vacuum_journal:100M");
}
...
async fn test_mock_docker_ops() {
    ...
    assert!(ops.compose_pull_current().is_ok());
    assert!(ops.compose_down(compose_file).is_ok());
    assert!(ops.prune_system().is_ok());
    assert!(ops.compose_up_remove_orphans().is_ok());

    let calls = ops.calls.lock().unwrap_or_else(|e| e.into_inner());
    assert_eq!(calls.len(), 8);
    assert_eq!(calls[0], "install");
    assert_eq!(calls[1], "is_daemon_running");
    assert_eq!(calls[2], "compose_up:docker-compose.yml");
    assert_eq!(calls[3], "compose_pull:docker-compose.yml");
    assert_eq!(calls[4], "compose_pull_current");
    assert_eq!(calls[5], "compose_down:docker-compose.yml");
    assert_eq!(calls[6], "prune_system");
    assert_eq!(calls[7], "compose_up_remove_orphans");
}
